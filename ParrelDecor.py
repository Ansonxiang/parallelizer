...
class parallelizer():
    ...
    def parallelize_arr(self, parrall_meter, axis=0, start=0):
        def decorator(func):
            def wrapper(*args,**kwargs):
                n_jobs = self.n_jobs
                if axis == 0:
                    ...
                    chunks=[]
                    for gp in groups:
                        sub_kwargs={k:v[list(range(0,start))+list([x+start for x in gp]),:] for k,v in kwargs.items() if k==parrall_meter}
                        sub_kwargs.update({k:v for k,v in kwargs.items() if k!=parrall_meter})
                        chunks.append(sub_kwargs)
